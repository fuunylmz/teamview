...
Expected output includes `codec-backend kind=video codec=H264 backend=synthetic available=true ...` and a Media Foundation H.264 hardware probe line. Use `--video-encoder media-foundation` to explicitly request that backend; the client now fails early with a capability/detail message if no hardware H.264 encoder MFT is available or if the backend has not yet wired frame submission.
...
To publish screen and voice from one broadcaster connection, use `--media-kind both`. The screen stream uses `--stream-id`; the voice stream defaults to the next id and can be set with `--voice-stream-id`. A viewer can also use `--media-kind both` to subscribe to both selected streams on one connection and demux packets by stream id.
