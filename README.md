...
For a scripted remote-input smoke, add `--remote-input-script pointer-tap` to a screen viewer. The viewer sends a bounded control-plane input event to the relay after subscribing, and the broadcaster polls and logs `remote-input` events while publishing the screen stream.
