...
Expected output includes startup `time-sync` lines with relay RTT and `clock_offset_micros`, the broadcaster publishing `StreamConfig`, setting target bitrate/framerate, printing `media-send` lines with capture/encode/packetize/send timing, the viewer polling config before media, publisher feedback polling, relay `stream-metrics`, received frames with `latency_ms`, `media-render` lines, periodic `viewer-stats` responses, final `unsubscribe-stream` / `leave-room` responses, and a final viewer summary similar to:
