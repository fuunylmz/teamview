...
To publish screen and voice from one broadcaster connection, use `--media-kind both`. The screen stream uses `--stream-id`; the voice stream defaults to the next id and can be set with `--voice-stream-id`. Viewers still subscribe to one media kind per process today, so run a screen viewer and a voice viewer separately when testing both streams.
...
cargo run -p desktop-client -- --mode viewer --relay 127.0.0.1:4433 --room-name stage1 --media-kind screen --stream-id 1 --media-run-ms 1000 --media-fps 30
cargo run -p desktop-client -- --mode viewer --relay 127.0.0.1:4433 --room-name stage1 --media-kind voice --stream-id 2 --media-run-ms 1000 --media-fps 30
```
