...
Open `http://127.0.0.1:7788/` to load the channel console. The channel list calls the client-local API to select the active relay channel for UI controls. In broadcaster mode, the screen share control publishes or unpublishes the relay screen stream in that selected channel and runs the screen media sender while sharing is active. In `--media-kind voice` or `--media-kind both`, the voice controls also publish the selected channel's voice stream and run the voice media sender while the local voice state is speaking; muting, deafening, or releasing inactive push-to-talk stops the sender and unpublishes that voice stream. For bridge/debug tooling, `--export-ui-state apps/desktop-ui/state.json` writes the same channel snapshot to disk and `--print-ui-state` prints it to stdout.
