...
Use `--muted` on a voice broadcaster to publish the room/stream state without sending voice frames. Use `--deafened` on a viewer to update relay voice state, suppress voice delivery, and avoid local audio playback.
