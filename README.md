...
Open `http://127.0.0.1:7788/` to load the channel console. The voice controls call the client-local API and update relay `SetVoiceState` for mute, deafen, and push-to-talk state before refreshing the channel snapshot. For bridge/debug tooling, `--export-ui-state apps/desktop-ui/state.json` writes the same channel snapshot to disk and `--print-ui-state` prints it to stdout.
