...
The desktop client also exposes a channel UI snapshot contract: `--export-ui-state` / `--print-ui-state` convert relay room, participant, screen stream, voice stream, local user id, and local voice/screen controls into the same camelCase JSON model consumed by `apps/desktop-ui`. `--serve-ui` embeds the same static console, serves `/state.json` from live relay discovery, accepts `POST /api/select-channel` so the channel list changes the client-local selected channel, accepts `POST /api/screen-share` so broadcaster-mode screen controls publish or unpublish the relay screen stream and run the screen media sender in that selected channel while sharing is active, and accepts `POST /api/voice-state` so broadcaster-mode voice controls update relay voice state, publish/unpublish the relay voice stream, and run or stop the voice media sender while the local voice state is speaking. This keeps the frontend aligned with the native client state while a production desktop shell is still being wired.
