...
The desktop QUIC control client is cloneable and shares a single atomic request-id sequence across clones. That keeps control responses unambiguous while preparing the client to split screen and voice media loops across concurrent tasks on the same QUIC connection.
The broadcaster now uses that shared client to publish screen and voice streams together with `--media-kind both`; viewer-side simultaneous multi-stream demux is still a later client milestone.
