...
Relay stream metrics include ingress/egress counters, drop counts, current egress queue packet/media depth, subscriber counts, last ingress time, and receive-to-route p50/p95 timing so publisher-side logs can separate server routing and queueing cost from broadcaster and viewer pipeline cost. Clients also issue a control-plane `TimeSync` to estimate relay clock offset before media starts. Forwarded media datagrams carry sender encode/send and relay receive/send timestamps so viewer logs can report publisher-side media timing and relay receive-to-send queue delay. Viewer stats split receive-to-reassembly p50/p95 from decode and render timing, which makes jitter/reorder delay visible before native hardware decode lands.
