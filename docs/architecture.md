...
- Room-scoped voice state is tracked by the relay. `--muted` suppresses client voice sending, and `--deafened` suppresses relay voice delivery plus local playback for that viewer.
