...
`Hello.client_name` currently doubles as the user's room display name until a full account/profile system exists. The relay trims and bounds that value, returns the assigned `user_id` and final display name from `HelloAccepted` and `Authenticated`, uses it in participant discovery, and falls back to `user-{id}` if the provided name is blank.
